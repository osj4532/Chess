...
public class ChessBoard {
	...
	public static ArrayList<ArrayList<Integer>> checkerRange = null;
	...
	public static void main(String[] args) {			
		...
		//Player whitePly = new AlphaBetaPlayer("white", 1);
		Player blackPly = new AlphaBetaPlayer("black", 2);
			...
			for(int i = 0 ; i < 8; i++) {
				for(int j = 0; j < 8; j++) {
					if(board[i][j].getType() == "pawn")
						System.out.println(board[i][j].aiPath());
				}
			}
			...
			for(int runNum = 1; runNum <= 2; runNum++){ //run for each player
				...
				while(true){
					...
					if(checkValue){
						...
						wKYX = new ArrayList<Integer>();
						bKYX = new ArrayList<Integer>();
						for(int i = 0; i < 8; i++) {
							for(int j = 0; j < 8; j++) {		
								if(board[i][j].getSymbol() == "WKi") {
									wKYX.addAll((Arrays.asList(i, j)));
								}else if(board[i][j].getSymbol() == "BKi") {
									bKYX.addAll((Arrays.asList(i, j)));
								}
							}
						}
						if(wKYX.isEmpty() || bKYX.isEmpty()) {draw(); return;}
