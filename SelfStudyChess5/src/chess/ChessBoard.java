...
public class ChessBoard {
	...
	private static void setup() {
			...
			board[1][6] = new Bishop("black",1,6);
	...
	private static void check() {
		...
		/*if(count == 1) {
			...
			for(int i = 0; i < 8; i++) {
				for(int j = 0; j < 8; j++) {
					...
					tmp.addAll(board[i][j].attackPathCheck());
				}
			}
		}*/
		...
		if(count == 1) {
			...
			for(int i = checkPoint - 1; i >= 0; i--) {
				if(Math.abs(allWAttackPath.get(i).get(0) - allWAttackPath.get(i + 1).get(0)) == 1 ||
						Math.abs(allWAttackPath.get(i).get(1) - allWAttackPath.get(i + 1).get(1)) == 1) {
					if((Math.abs(allWAttackPath.get(i).get(0) - allWAttackPath.get(i + 1).get(0)) > 1||
							Math.abs(allWAttackPath.get(i).get(1) - allWAttackPath.get(i + 1).get(1)) > 1)) break; 
					tmp2 = new ArrayList<Integer>(Arrays.asList(allWAttackPath.get(i).get(0), allWAttackPath.get(i).get(1)));
					tmp.add(tmp2);
				} 
			}
				...
				for(int j = 0; j < 8; j++) {
					if(board[i][j].getType() != "none" && board[i][j].getColor() == "black" && 
						...
						for(int k = 0; k < tmp.size(); k ++) {
							if(board[i][j].attackPathCheck().contains(tmp.get(k))) {
								System.out.println("hello!!" + i + j + tmp.get(k));
							}
		...
		if(count == 1) {
			tmp = new ArrayList<ArrayList<Integer>>();
			for(int i = checkPoint - 1; i >= 1; i--) {
				if(Math.abs(allBAttackPath.get(i).get(0) - allBAttackPath.get(i - 1).get(0)) == 1 ||
						Math.abs(allBAttackPath.get(i).get(1) - allBAttackPath.get(i - 1).get(1)) == 1) {
					if((Math.abs(allBAttackPath.get(i).get(0) - allBAttackPath.get(i - 1).get(0)) > 1||
							Math.abs(allBAttackPath.get(i).get(1) - allBAttackPath.get(i - 1).get(1)) > 1)) break; 
					
					tmp2 = new ArrayList<Integer>(Arrays.asList(allBAttackPath.get(i).get(0), allBAttackPath.get(i).get(1)));
					tmp.add(tmp2);
				} 
			}
		}
